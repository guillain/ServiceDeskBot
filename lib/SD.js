...
exports.join = function(bot, trigger, id) {
  ...
  room = spark.roomAdd(config.SD.roomtitle)
    .then(function(room) {
      bot.say(config.msg.roomcreationok+' - '+room.title);
      ...
      msgsend = spark.messageSend({roomId: room.id, text: tosay, markdown: tosay})
      ...
      itsm = ITSM.create(bot, trigger, config.msg.istmcontent, id);
    })
    .catch(function(err) { 
      bot.say(config.msg.roomcreationerr); console.log(err); 

      // Create ITSM ticket to log current room creation issue
      itsm = ITSM.create(bot, trigger, config.msg.roomcreationissue, id);
    });
};
